...
from im2mesh.occ_scf_net.models import vnn_occupancy_scf_net, vnn_occupancy_net_pointnet_dgcnn
...
model = vnn_occupancy_net_pointnet_dgcnn.VNNOccNet(latent_dim=256, sigmoid=False, return_features=True).cuda()

# load weight
# model_path = "model_final.pth"
model_path = "ndf_mug.pth"
model_dict = torch.load(model_path)

model.load_state_dict(torch.load(model_path))
model.eval()

val_dataset = JointOccScfTrainDataset(128, o_dim=5, obj_class="mug", multiview_aug=False, phase="val")
...
with torch.no_grad():
    for step, (model_input, gt) in enumerate(val_dataloader):
        ...
        box_size = np.max(bb.bounds - bb.centroid) * 3.0
        ...
        mesh = trimesh.Trimesh(vertices, triangles,
                               ...
                               process=False)
        pcd = test_input["point_cloud"].squeeze(0).cpu().numpy() + np.array([0.3, 0, 0])
